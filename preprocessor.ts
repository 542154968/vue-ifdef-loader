...
class IfBlock {
   ...
   */
  constructor(
    public line_if: number,
    public line_endif: number,
    public elifs: number[] = [],
    public line_else: number | null = null,
    public inner_ifs: IfBlock[] = []
  ) {}
...
export function parse(
  ...
  uncommentPrefixString?: string
): string {
  if (tripleSlash === undefined) tripleSlash = true;
  useTripleSlash = tripleSlash;

  if (fillWithBlanks === undefined) fillWithBlanks = true;
...
// 为了过滤掉<!-- --> 低成本支持 vue template
const htmlCommentsIdentificationRegex = /<!--|-->/g;

function replaceHtmlCommentsIdentification(line) {
  return line.replace(htmlCommentsIdentificationRegex, "");
}

function match_if(line: string, type: IfType = IfType.If): boolean {
  const re = ifRegex();
  const match = re.exec(replaceHtmlCommentsIdentification(line));
  return (
    match !== null &&
...
function apply_if(
  ...
  const log = (condition: string, outcome: boolean) => {
    if (verbose) {
      console.log(
        `#if block lines [${ifBlock.line_if + 1}-${
          ifBlock.line_endif + 1
        }]: Condition '${condition}' is ${outcome ? "TRUE" : "FALSE"}. ${
          includeRange != null
            ? `Including lines [${includeRange.from + 1}-${
                includeRange.to + 1
              }]`
